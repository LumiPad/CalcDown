# CalcDown 1.1 (Specification)

Status: **1.1**. CalcDown is a text-first, Git-friendly format for “spreadsheet-like” models: typed inputs and data, a deterministic compute graph, and declarative views.

This document specifies **CalcDown 1.1** (the file format, project files, execution model, Editor Protocol, and expected tooling). The companion standard library is specified in `docs/stdlib-1.1.md`.

This spec supersedes `docs/calcdown-1.0.md` and is a consolidation of the archived `0.x` drafts under `docs/calcdown-0.*.md`.
...
- Implementations SHOULD accept `calcdown` as either a YAML number (`1.1`) or a string (`"1.1"`), but MUST treat it as an exact version identifier (not a range).
...
CalcDown 1.1 standardizes an optional manifest file to describe a multi-document project without relying solely on per-document front matter.

The manifest MUST be JSON and MUST be a single JSON object.

CalcDown ships a JSON Schema for the manifest:

- `schemas/calcdown-manifest-1.1.schema.json`
...
- `calcdown` (string|number, optional): the spec version (recommended `"1.1"`).
...
CalcDown 1.1 standardizes an optional lockfile to support reproducible review and CI checks.

The lockfile MUST be JSON and MUST be a single JSON object.

CalcDown ships a JSON Schema for the lockfile:

- `schemas/calcdown-lock-1.1.schema.json`

Required keys:

- `calcdown` (string, required): `"1.1"` (lockfile schema version).
...
CalcDown 1.1 defines the following block types (by code-fence language tag):
...
```
<name> : <type> = <default> [<constraints>] [# comment]
```
...
- If present, `<constraints>` MUST be a single bracketed list (e.g. `[min: 0, max: 100]`).
- Engines SHOULD enforce `min`/`max` constraints when rendering input UIs and validating overrides.
...
CalcScript expressions (1.1) support:

- Literals: numbers, strings, booleans
- Identifiers
- Object literals (`{ a: 1, b }`)
- Arrow functions (`(x) => x + 1`, `({ a, b }) => a + b`) for use as arguments to `std.*` APIs
- Member access (`obj.key`) and calls (`std.math.sum(xs)`), subject to the safety rules above
- Array indexing (`xs[i]`) where `xs` is an array and `i` is an integer scalar
- Operators:
  - Unary:
...
- For index access `xs[i]`:
  - `xs` MUST evaluate to an array.
  - `i` MUST evaluate to a non-negative integer scalar.
  - Index access MUST throw if `i` is out of bounds.
...
CalcDown 1.1 adds an optional convenience: engines MAY recognize **table patch statements** inside `calc` blocks to update inline `data` tables (typically used as “output templates”).
...
CalcDown 1.1 standardizes view types under `library: "calcdown"`:
...
- `schemas/calcdown-view-1.1.schema.json`
- `schemas/calcdown-view-cards-1.1.schema.json`
- `schemas/calcdown-view-table-1.1.schema.json`
- `schemas/calcdown-view-chart-1.1.schema.json`
- `schemas/calcdown-view-layout-1.1.schema.json`

CalcDown 1.1 standardizes a small ergonomic default:
...
CalcDown 1.1 extends the standardized `chart` view to support **multiple plotted series**.

CalcDown 1.1 extends the standardized `table` view columns with optional **conditional formatting**:

- `conditionalFormat`: a list of rules evaluated in order.
  - `when`: a CalcScript expression evaluated per cell with `value` bound to the cell value and `row` bound to the row object.
  - `style`: either a built-in style token (`positive`, `negative`, `neutral`, `warning`, `highlight`) or an inline style object with a small allowed subset of CSS properties (see schema).
...
- `schemas/calcdown-export-1.1.schema.json`
...
## Appendix A) Changes from 1.0 → 1.1

- Adds CalcScript array indexing (`xs[i]`).
- Adds object destructuring parameters in arrow functions (`({ a, b }) => ...`).
- Adds optional input constraints (`[min: ..., max: ...]`) in `inputs` blocks.
- Adds table column conditional formatting (`conditionalFormat`) in `view` table columns.
