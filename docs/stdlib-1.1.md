# CalcDown Standard Library 1.1

Status: **1.1**. This document specifies the standard library object available as `std` when evaluating CalcScript expressions in CalcDown 1.1.

CalcDown 1.1 expands the standard library with additional finance and text helpers.

See also: `docs/calcdown-1.1.md` (the file format, project files, execution model, and tooling).
...
A CalcDown 1.1 engine MUST provide a `std` object with the **Core** APIs in §3.
...
#### `std.finance.ipmt(rate, per, nper, pv, fv?, type?)`

Excel-compatible interest portion of a payment for a given period (IPMT).

Rules:

- `per` MUST be an integer in `[1, nper]`.
- For `type = 1` (payments at beginning), `ipmt(..., per=1, ...)` MUST be `0`.

#### `std.finance.ppmt(rate, per, nper, pv, fv?, type?)`

Excel-compatible principal portion of a payment for a given period (PPMT).

Rules:

- `ppmt(...)` MUST satisfy `ppmt(...) == pmt(...) - ipmt(...)` for the same arguments.

#### `std.finance.npv(rate, cashflows)`

Net present value (NPV) of periodic cashflows.

Rules:

- `cashflows` MUST be an array of finite numbers.
- Cashflows MUST be discounted as period-end flows (the first element is discounted by `(1 + rate) ** 1`).

#### `std.finance.irr(cashflows, guess?)`

Internal rate of return (IRR) for periodic cashflows.

Rules:

- `cashflows` MUST be an array of finite numbers.
- Cashflows MUST include at least one positive and at least one negative value.
...
Uppercase a string (or each string in an array).

#### `std.text.lower(value)`

Lowercase a string (or each string in an array).

#### `std.text.trim(value)`

Trim leading/trailing whitespace from a string (or each string in an array).

#### `std.text.length(value)`

Return the length of a string (or each string in an array).

#### `std.text.slice(value, start, end?)`

Return a substring (or substrings) by slicing with integer indices.

#### `std.text.split(value, sep)`

Split a string by a literal separator.

#### `std.text.startsWith(value, prefix)`
...
#### `std.text.padStart(value, targetLength, padString?)`

Left-pad a string to a given target length.

#### `std.text.format(template, ...args)`

Deterministic placeholder formatting for `{0}`, `{1}`, … using scalar and vector broadcasting semantics (like `std.text.concat`).
...
- Adds `std.finance.ipmt`, `std.finance.ppmt`, `std.finance.npv`, and `std.finance.irr`.
- Expands `std.text` beyond `concat`/`repeat` with basic string helpers (upper/lower/trim/length/slice/split/startsWith/contains/padStart/format).
